...
async function writeOnce(intent, operation) {
  ...
  return withLock(`operation:${operationId}`, async () => {
    ...
    if (previous && previous.status === 'prepared') {
      // Proof-based recovery, not an age-based unlock: the submit() IIFE only runs action()
      // AFTER the 'submitted' record is durably written, so a record still reading 'prepared'
      // (under this operation lock) proves no external side effect ever started.
      previous.reviews = [...(previous.reviews || []), { decision: 'auto_recovered_stale_prepared', mode: 'proof_based_not_age_based',
        reason: 'prepared 状态证明提交动作从未开始（submitted 先落盘、后执行副作用），允许重跑',
        reviewedAt: new Date().toISOString(), previousState: 'prepared' }];
    }
    if (previous && !['failed_before_submit', 'retry_authorized', 'prepared'].includes(previous.status)) {
