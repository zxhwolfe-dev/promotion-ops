...
async function writeOnce(intent, operation) {
  ...
  return withLock(`operation:${operationId}`, async () => {
    ...
    // A historical prepared record is NOT a storage-proven absence of a remote write.
    // Awaiting writeJSON gates action() in a live execution, but older snapshots can
    // reappear after a system/storage failure (Windows has no directory sync here).
    // Also, operation() may trigger editor autosaves outside submit(). Never infer
    // zero external effects from this status alone; retain the explicit reconcile path.
