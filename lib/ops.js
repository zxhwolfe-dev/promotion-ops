...
async function run(main) {
  try {
    ...
    if (!result || typeof result !== 'object' || Array.isArray(result)) throw new OpsError('NO_RESULT', '脚本未返回结构化结果');
    if (!['ok', 'verified', 'already_verified', 'partial', 'unavailable'].includes(result.status)) {
      throw new OpsError('INVALID_RESULT_STATUS', '脚本未返回已知状态，不能按成功退出');
    }
    ...
    process.exitCode = [1, 2, 3, 4].includes(error?.exitCode) ? error.exitCode : 1; // 不使用 process.exit：先完成 finally 清理。
  }
}
...
async function readText(file, maxBytes = 2 * 1024 * 1024) {
  ...
  try {
    ...
    // A single bounded read also caps files that grow after stat(). Reject malformed UTF-8.
    ...
    if (size > maxBytes) throw new OpsError('INVALID_FILE', '输入超过允许大小');
    let text;
    try { text = new TextDecoder('utf-8', { fatal: true }).decode(buffer.subarray(0, size)); }
    catch { throw new OpsError('INVALID_UTF8', '输入不是有效 UTF-8，请先明确转换编码'); }
...
async function fillEmpty(locator, value, { keyboard = false } = {}) {
  // contenteditable 编辑器（如 CSDN 的 markdown pre）会用渲染后的 innerText 吃掉标题前空行；
  // 回读一律用 textContent 取原始文本，比对才可靠。
  const old = await locator.evaluate(el => 'value' in el ? el.value : el.textContent);
  ...
  const actual = await locator.evaluate(el => 'value' in el ? el.value : el.textContent);
  // markdown 编辑器会把连续换行规范化为单个换行，逐字符比较必然失败；
  // 回读断言采用全空白无关口径（仍能抓住漏填、重复、乱序）。
  if (actual.replace(/\s+/g, '') !== value.replace(/\s+/g, '')) throw new OpsError('FILL_MISMATCH', '编辑器回读与输入不一致，停止发布');
}
