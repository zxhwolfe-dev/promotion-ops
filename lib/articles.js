...
async function insertEmpty(page, locator, text) {
  const before = await locator.evaluate(el => 'value' in el ? el.value : el.textContent);
  if (normalize(before)) throw new OpsError('EXISTING_DRAFT', '编辑器已有草稿，拒绝覆盖；请人工处理', 3);
  await locator.click();
  await page.keyboard.insertText(text);
  const after = await locator.evaluate(el => 'value' in el ? el.value : el.textContent);
  if (after.replace(/\s+/g, '') !== text.replace(/\s+/g, '')) throw new OpsError('FILL_MISMATCH', '正文回读不一致，停止发布');
}
